...
- **Progress storage: Supabase**, free tier — a dedicated Supabase **Project** named `family_hub`, living in James's existing personal `beja0502` org (the same org that already hosts the separate, isolated `Forge Anchor` production project as its own project — not a shared database; `Reverie Revival` lives under a different, business email entirely). `beja0502`'s org-level free tier allows two active free projects; Forge Anchor uses one, `family_hub` is the second. One dedicated schema inside it: `python_learning`. This same `family_hub` project is expected to also eventually host a `meridian_almanac` schema for a separate personal project — both are expected to stay small enough to share one free-tier project comfortably.
