...
## Addendum (2026-07-19) — Errors panel gets a static hint; Help/Reset live elsewhere
Added a "How To Fix It" block beneath the raw traceback in the Errors panel — a plain-English translation of the error, looked up by error type (`js/errorHints.js`), always shown alongside the crash, not a button or a tab. Consistent with the "no extra click" reasoning above.

Help/Reset (fill in known-correct code / undo that) were briefly prototyped *in* the Errors panel, then moved out once real testing surfaced that they solve a different problem: a practice check can fail with code that ran fine but produced the wrong output — no exception, so the Errors panel never appears at all. Help/Reset now live in the practice-failure message inside the Output pane, where a stuck kid actually is. The Errors panel stays reserved for genuine crashes only, per the original addendum's scope.
