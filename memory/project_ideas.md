...
- **Chapter welcome page + hierarchical dropdown + real lesson locking.** New profiles now land on a "👋 Welcome" page for Chapter 1 before any lesson. The header dropdown groups lessons under the chapter name, shows real titles + ✓ for completed ones, and genuinely disables (🔒) any lesson whose predecessor isn't done yet — you cannot select a locked lesson. Next is also blocked on a practice lesson until its check passes, so Next/Prev and the dropdown never contradict each other. This resolves the lesson-to-lesson half of the old "Parent Gate" item (see #3 above for what's still open). Built and verified via automated browser test 2026-07-18 — needs James to try it himself: visit a fresh/new profile, confirm it lands on Welcome, confirm 1.2 shows locked (🔒) in the dropdown until 1.1 is finished, and confirm Next won't advance past 1.2 until the practice is passed.
