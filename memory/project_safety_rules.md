...
- Row Level Security on the `code_school` schema must keep each kid's progress rows isolated from the other's, even without real login — treat RLS policy correctness as a real requirement, not boilerplate.
